...
import lat.elpainauchoco.lisalisa.data.game.ArtifactSetData;
...
public class ArtifactConfSanitiser {
    ...
    private final String pieceUID;
    private final String setId;
    private final ArtifactSetData artifactSet;
    private ArtifactRarityData rarityData;

    public ArtifactConfSanitiser(final GameDataService gservice, final ArtifactPieceConf piece, final String pieceType, final String pieceUid) {
        this.gservice = gservice;
        this.piece = piece;
        this.pieceType = pieceType;
        this.pieceUID = pieceUid;
        this.setId = piece.getSet();
        if(!gservice.hasArtifactSet(setId)) {
            throw new UserConfigException("Invalid artifact set " + setId + " for artifact piece " + pieceUid);
        }
        this.artifactSet = gservice.getArtifactSet(setId);
    }
    ...
    protected void sanitiseType(final String type) {
        if(!artifactSet.getPieces().contains(type)) {
            throw new UserConfigException("Invalid type " + pieceType
                    + " for artifact piece " + pieceUID
                    + " of set " + setId
            );
        }
    ...
    protected void sanitiseRarity(final int rarity) {
        if(!artifactSet.getRarities().contains(rarity)) {
            throw new UserConfigException("Invalid rarity " + rarity
                    + " for artifact piece " + pieceUID
                    + " of set " + setId
            );
        }
    ...
    protected void sanitiseLevel(final int level) {
        if(level < gservice.getMinArtLevel()) {
            throw new UserConfigException("Invalid rarity " + level + " for artifact piece " + pieceUID);
        }
    ...
    protected void sanitiseSubStats(final Map<String, Double> substats, final String mainStat, final int level) {
        if(substats.size() > rarityData.getMaxSubstats()) {
            throw new UserConfigException("Artifact piece " + pieceUID + " has too much substats");
        }
        if(substats.size() < rarityData.getMaxSubstats() - (level < 4 ? 1 : 0)) {
            throw new UserConfigException("Artifact piece " + pieceUID + " has too little substats");
        }
        if(substats.containsKey(mainStat)) {
            throw new UserConfigException("Artifact piece " + pieceUID + " has its main stat as a substat");
        }
        ...
        for(Map.Entry<String, Double> e : substats.entrySet()) {
            if(!rarityData.getSubstats().contains(e.getKey())) {
                throw new UserConfigException("Invalid substat " + e.getKey() + " in artifact piece " + pieceUID);
            }
